...
void cHardwareExperimental::LookResults(sLookRegAssign& regs, cOrgSensor::sLookOut& results)
{
  ...
  if (m_world->GetConfig().LOOK_DISABLE_COMBO.Get() > 0 && results.report_type == 1) {
	...
	results.report_type = 0; // predator
      ...
      else if (disable_combo == 3 && results.habitat == -2 && (results.search_type == -1 || results.search_type == -2))  {
	results.report_type = 0; // prey
  	results.id_sought = -1;
      }
      else if (disable_combo == 2 && (results.habitat == -2 || results.habitat == 5) && results.forage <= -2)  {
	results.report_type = 0; // predator
  	results.id_sought = -1;
      }
      else if (disable_combo == 3 && results.habitat == -2 && results.forage > -2)  {
	results.report_type = 0; // prey
  	results.id_sought = -1;
      }
    }
...
bool cHardwareExperimental::DoActualCollect(cAvidaContext& ctx, int bin_used, bool unit)
{
  ...
  tArray<double> res_change(m_world->GetEnvironment().GetResourceLib().GetSize());
