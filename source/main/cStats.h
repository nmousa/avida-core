...
protected:
  ...
  struct message_log_entry_t {
    message_log_entry_t(int u, int de, int s, int d, int t, unsigned int md, unsigned int ml, int lt, int dt, bool dr, bool l)
      :	update(u), deme(de), src_cell(s), dst_cell(d), transmit_cell(t), msg_data(md), msg_label(ml), label_task_id(lt), data_task_id(dt), dropped(dr), lost(l) {
    }
    int update, deme, src_cell, dst_cell, transmit_cell, label_task_id, data_task_id;
